...
export default function App() {
  ...
  const handleResetAll = async () => {
    // Iterate channels and turn off all (including ECU)
    for (const ch of channels) {
      if (ch.status?.on) {
        try { await pdmApi.controlChannel(ch.id, false); } catch {}
      }
    }
  };
  ...
  const visibleChannels = channels.filter(ch => visibleChannelIds.includes(ch.id));

  // Derived - calculate from VISIBLE channels only
  const totalCurrent = visibleChannels.reduce((sum, ch) => sum + (ch.current || 0), 0);
