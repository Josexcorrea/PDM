...
class STM32Simulator:
    ...
    def __init__(self, port: str = "COM10", baud: int = 115200):
        ...
        self.channels_state: List[Dict] = [
            {"voltage": 10.0, "current": 0.3, "temp": 45.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 3.5, "temp": 38.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 4.8, "temp": 42.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 8.0, "temp": 35.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 1.2, "temp": 32.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 0.8, "temp": 30.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 2.1, "temp": 35.0, "status": 0b00000001},
            {"voltage": 10.0, "current": 0.0, "temp": 28.0, "status": 0b00000000},
        ]
    ...
    def _update_sensors(self):
        ...
        self.voltage = max(9.8, min(10.2, self.voltage))
        ...
        for i, ch in enumerate(self.channels_state):
            ...
            if is_on:
                if i == 7:  # Starter
                    ...
                    base = [0.3, 3.5, 4.8, 8.0, 1.2, 0.8, 2.1, 100.0][i]
                    ch["current"] = max(0.1, base + random.uniform(-0.05, 0.05))
            else:
                ch["current"] = 0.0
